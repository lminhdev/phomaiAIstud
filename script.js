...
function isDocumentWideRequest(query, taskKind = "") {
  const q = normalizeText(query).toLowerCase();
  if (["summary", "explain", "quiz", "flashcard", "outline", "exam"].includes(taskKind)) return true;
  return /(tóm\s*tắt|tom\s*tat|tạo\s*(quiz|câu hỏi|flashcard|đề)|giải thích\s*(toàn|nội dung)|dàn ý|toàn bộ|tài liệu đang chọn|file\s*(tôi|tớ|mình|t)\s*(đã )?chọn|file\s*t\s*chọn)/iu.test(q);
}

function buildContext(query, taskKind = "") {
  const selected = getSelectedDocument();

  // IMPORTANT: document-wide tasks must not rely on lexical retrieval.
  // Phrases such as "tóm tắt file t chọn" do not occur inside the PDF,
  // so lexical RAG would otherwise return zero chunks.
  if (selected && isDocumentWideRequest(query, taskKind)) {
    ensureStats(selected);
    let text = "";
    const citations = [];

    for (let i = 0; i < (selected.chunks || []).length; i++) {
      const h = selected.chunks[i];
      const tag = `[Nguồn ${i + 1}: ${selected.name}${h.page ? `, trang ${h.page}${h.endPage && h.endPage !== h.page ? "-" + h.endPage : ""}` : ""}]`;
      const block = `${tag}\n${h.text}\n\n`;
      if (text.length + block.length > CONFIG.MAX_CONTEXT) break;
      text += block;
      citations.push({ n: i + 1, docName: selected.name, page: h.page, text: h.text });
    }

    if (text) {
      return { text, citations, documentWide: true, documentName: selected.name };
    }
  }

  // Normal chat: use lexical retrieval against the selected document first.
  const hits = retrieve(query, selectedDocumentId ? [selectedDocumentId] : null);

  // Fallback: if a document is selected but lexical search found nothing,
  // provide a small set of chunks instead of sending an empty context.
  if (!hits.length && selected) {
    ensureStats(selected);
    const fallback = (selected.chunks || []).slice(0, CONFIG.TOP_K).map((h, i) => ({
      ...h,
      docId: selected.id,
    ...
    return { text, citations, documentWide: false, documentName: selected.name };
  }
  ...
  return { text, citations, documentWide: false, documentName: selected?.name || null };
}
...
- Khi có CONTEXT từ tài liệu: bắt buộc ưu tiên và sử dụng CONTEXT để trả lời.
- Nếu CONTEXT được đánh dấu là tài liệu đang chọn và yêu cầu là tóm tắt/quiz/flashcard/dàn ý/giải thích, hãy xử lý trực tiếp nội dung đã cung cấp, không được nói "chưa nhận được file".
- Nếu CONTEXT không đủ thông tin cho một câu hỏi cụ thể, nói rõ "Tài liệu hiện có không đủ thông tin để kết luận".
...
async function sendMessage(text, taskKind = "") {
  enforceAutoModel();
  text = text.trim(); if (!text) return;
  if (!apiKey || apiKey.startsWith("PASTE_")) { openSettings(); showToast("Groq API key chưa được cấu hình."); return; }
  $("#input").value = ""; $("#send").disabled = true;
  addMessage("user", text);
  const context = buildContext(text, taskKind);
  ...
  try {
    ...
    if (context.citations.length) {
      ...
      meta.textContent = `📚 RAG: ${names}${context.documentWide ? " • toàn bộ tài liệu" : ""}`;
...
$$("[data-prompt]").forEach(b => b.addEventListener("click", () => {
  ...
  sendMessage(b.dataset.prompt, b.dataset.task || "");
}));
...
$$(".tool").forEach(b => b.addEventListener("click", () => {
  ...
  switchPage("chat");
  $("#input").value = prompt;
  sendMessage(prompt, b.dataset.tool || "");
}));
...
function init() {
  ...
  renderLibrary();
  restoreChat();
  if (apiKey) setStatus("Groq • GPT-OSS 120B", true);
