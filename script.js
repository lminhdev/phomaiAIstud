...
function updateModelInfo() {
  const info = CONFIG.MODELS[$("#modelSelect").value];
...
function retrieve(query) {
  ...
  const stats =
    selectedDocument.stats ||
    buildDocumentStats(selectedDocument.chunks);
...
Kiểm tra WebGPU hoặc thử model 1B nếu thiết bị không đủ tài nguyên.`
    );
  } finally {
    send.disabled = false;
  }
